    ...
    <div className="min-h-screen bg-background">
      ...
      >
        ...
        <div className="absolute inset-0 bg-background">
          <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff08_1px,transparent_1px),linear-gradient(to_bottom,#ffffff08_1px,transparent_1px)] bg-[size:40px_40px] md:bg-[size:60px_60px] lg:bg-[size:80px_80px]" />
          
          {/* Animated gradient orbs (lime + charcoal) */}
          <div 
            className="absolute top-1/4 left-1/4 w-96 h-96 bg-gradient-to-r from-lime-400/20 to-lime-500/10 rounded-full blur-3xl animate-pulse"
            style={{
              transform: `translate(${mousePosition.x * 0.01}px, ${mousePosition.y * 0.01}px)`
            }}
          />
          <div 
            className="absolute bottom-1/4 right-1/4 w-96 h-96 bg-gradient-to-r from-zinc-600/20 to-zinc-400/10 rounded-full blur-3xl animate-pulse"
            style={{
              transform: `translate(${-mousePosition.x * 0.01}px, ${-mousePosition.y * 0.01}px)`
            }}
          />
        ...
        <div className="relative z-10 text-center px-4 sm:px-6 max-w-6xl mx-auto animate-fade-in">
          ...
          <div className="mb-8 sm:mb-12">
            <p className="text-xs sm:text-sm uppercase tracking-[0.2em] text-muted-foreground mb-4">Trusted by teams at</p>
            <div className="relative overflow-hidden">
              <div className="marquee opacity-70">
                <span className="mx-6 text-foreground/70">FinTech Co.</span>
                <span className="mx-6 text-foreground/70">HealthAI</span>
                <span className="mx-6 text-foreground/70">RetailCloud</span>
                <span className="mx-6 text-foreground/70">StudioX</span>
                <span className="mx-6 text-foreground/70">CinePro</span>
                <span className="mx-6 text-foreground/70">DataForge</span>
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-bold leading-tight mb-4 sm:mb-6">
            <span className="block mb-2">Ship Faster.</span>
            <span className="relative inline-block">
              Scale Smarter.
              <span className="absolute -bottom-2 left-0 right-0 h-2 bg-gradient-to-r from-lime-400 via-lime-500 to-lime-400 opacity-20 blur-sm animate-pulse" />
            </span>
          </h1>
          ...
          <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-muted-foreground mb-8 sm:mb-12 max-w-3xl mx-auto leading-relaxed px-4">
            IT services and content at one studio: DevOps, networking, and cloud engineering alongside CG ads, editing, and motion graphics.
          </p>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center px-4">
              ...
              variant="default" 
              asChild 
              className="group shadow-lg hover:shadow-2xl hover:shadow-lime-400/20 w-full sm:w-auto text-sm sm:text-base px-8 sm:px-10 py-6 rounded-full transform hover:scale-105 transition-all duration-300"
            >
              <Link to="/portfolio">
                Explore Our Work
          ...
          <div 
            id="stats-section"
            className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mt-12 sm:mt-16 md:mt-20 pt-8 sm:pt-12 border-t border-border/50 px-4 max-w-4xl mx-auto"
          >
            <div className="animate-slide-up group" style={{ animationDelay: '0.1s' }}>
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-gradient-blue-cyan mb-1 sm:mb-2 transition-all duration-300 group-hover:scale-110">
                {counters.projects}+
              </div>
            ...
            <div className="animate-slide-up group" style={{ animationDelay: '0.3s' }}>
              <div className="text-2xl sm:text-3xl md:text-4xl font-bold text-gradient-blue-cyan mb-1 sm:mb-2 transition-all duration-300 group-hover:scale-110">
                {counters.years}+
              </div>
      ...
      <section className="px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24">
        <div className="container mx-auto">
          <div className="max-w-5xl mx-auto p-[1px] rounded-3xl bg-gradient-to-br from-lime-500/30 via-lime-400/10 to-zinc-700/40">
            <div className="relative overflow-hidden rounded-[calc(theme(borderRadius.3xl)-1px)] border border-border bg-gradient-to-br from-lime-400/10 via-transparent to-zinc-900 p-8 sm:p-12 creative-shadow">
              <div className="max-w-3xl mx-auto text-center">
              <h3 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-3">Ready to outpace the competition?</h3>
              <p className="text-muted-foreground mb-6 sm:mb-8">Let’s build a product that performs and a story that sells. Start with a free strategy call.</p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                <Button size="lg" asChild className="rounded-full px-8 py-6">
                  <Link to="/contact">Book Strategy Call</Link>
                </Button>
                <Button size="lg" variant="outline" asChild className="rounded-full px-8 py-6 border-2">
                  <Link to="/portfolio">See Case Studies</Link>
                </Button>
              </div>
              </div>
              <div className="pointer-events-none absolute -right-8 -bottom-8 w-72 h-72 rounded-full bg-lime-400/10 blur-3xl" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 sm:py-24 md:py-28 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        <div className="container mx-auto">
          <div className="text-center mb-12 sm:mb-16 animate-fade-in">
            ...
            <p className="text-muted-foreground max-w-2xl mx-auto">Results, not promises. Here’s why partners choose us again.</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 md:gap-8">
            {featuredProjects.map((project, index) => (
              <Link
                key={project.id}
                to="/portfolio"
              className="group relative overflow-hidden rounded-3xl aspect-[4/3] animate-slide-up hover-scale creative-shadow will-change-transform"
              onMouseMove={(e) => {
